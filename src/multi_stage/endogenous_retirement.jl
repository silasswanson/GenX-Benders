...
function compute_min_cumulative_retirements!(t::Int,inputs_d::Dict)
	...
	if !isempty(inputs_d[1]["STOR_ALL"])
		inputs_d[t]["dfGen"][!,Symbol("Min_Cum_Retired_Energy_Cap_MW")] = sum(inputs_d[p]["dfGen"][!,Symbol("Min_Retired_Energy_Cap_MW")] for p=1:t)
	end

	if !isempty(inputs_d[1]["STOR_ASYMMETRIC"])
		inputs_d[t]["dfGen"][!,Symbol("Min_Cum_Retired_Charge_Cap_MW")] = sum(inputs_d[p]["dfGen"][!,Symbol("Min_Retired_Charge_Cap_MW")] for p=1:t)
	end
...
function endogenous_retirement_discharge!(EP::Model, inputs::Dict, num_stages::Int, cur_stage::Int, stage_lens::Array{Int, 1})
	...
	@expression(EP, eMinRetCapTrack[y in RET_CAP],dfGen[y,Symbol("Min_Retired_Cap_MW")]/dfGen[y,:Cap_Size])
...
function endogenous_retirement_charge!(EP::Model, inputs::Dict, num_stages::Int, cur_stage::Int, stage_lens::Array{Int, 1})
	...
	@expression(EP, eMinRetCapTrackCharge[y in RET_CAP_CHARGE], dfGen[y,Symbol("Min_Retired_Charge_Cap_MW")]/dfGen[y,:Cap_Size])#sum((dfGen[!,Symbol("Min_Retired_Charge_Cap_MW")][y]) for p=1:cur_stage))
...
function endogenous_retirement_energy!(EP::Model, inputs::Dict, num_stages::Int, cur_stage::Int, stage_lens::Array{Int, 1})
	...
	@expression(EP, eMinRetCapTrackEnergy[y in RET_CAP_ENERGY], dfGen[y,Symbol("Min_Retired_Energy_Cap_MW")]/dfGen[y,:Cap_Size])#sum((dfGen[!,Symbol("Min_Retired_Energy_Cap_MW")][y]) for p=1:cur_stage))
