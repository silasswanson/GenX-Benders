...
function load_network_data!(setup::Dict, path::AbstractString, inputs_nw::Dict)
    ...
    if in("Line_Size_MW",names(network_var))
        inputs_nw["pLine_Size"] = to_floats(:Line_Size_MW)/ scale_factor #convert to GW
    else
        @info "Line_Size not found in Network.csv. Defaulting to  1.0 $(setup["ParameterScale"] == 1 ? "GW" : "MW") for all lines."
        inputs_nw["pLine_Size"] = ones(Float64, L)
    end   
    ...
    filename = "Maximum_path_transmission.csv"
    if isfile(joinpath(path, filename))
        df = load_dataframe(joinpath(path, filename))
        inputs_nw["dfMaxPathTrans"] = df
        inputs_nw["dfMaxPathTrans"][!,:Path_Max_Reinforcement_MW] ./= scale_factor 
        rename!(inputs_nw["dfMaxPathTrans"],:Path_Max_Reinforcement_MW => :Path_Max_Reinforcement)

        if setup["MultiStage"] >= 1
            inputs_nw["dfMaxPathTrans"][!,:Path_Max_Flow_Possible_MW] ./=scale_factor
            rename!(inputs_nw["dfMaxPathTrans"],:Path_Max_Flow_Possible_MW => :Path_Max_Flow_Possible)
        end
        inputs_nw["TransmissionPath"] = network_var[!,:transmission_path_name];
    end   
