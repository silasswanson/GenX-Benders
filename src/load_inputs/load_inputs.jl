...
function load_inputs(setup::Dict,path::AbstractString)
	...
	inputs["NumScenarios"] = setup["NumScenarios"];
	if inputs["NumScenarios"]>1 
		inputs["scenario_probability"] = ones(inputs["NumScenarios"]);
		if isfile(joinpath(path,"probability_distribution.csv"))
			assign_scenarios_weights!(setup,path,inputs)
		end
	end
...
function assign_scenarios_weights!(setup::Dict, path::AbstractString, inputs::Dict)

	println("Assigning a probability to each scenario")
	filename = "probability_distribution.csv"
    df = load_dataframe(joinpath(path, filename))
	scenario_times = timeseries2scenarios(inputs["T"], inputs["NumScenarios"]);
	for s=1:inputs["NumScenarios"]
		inputs["scenario_probability"][s] = df[1,Symbol("Scenario_$s")];
		inputs["omega"][scenario_times[s,1]:scenario_times[s,2]] *= df[1,Symbol("Scenario_$s")];
	end
