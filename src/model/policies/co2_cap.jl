...
function co2_cap!(EP::Model, inputs::Dict, setup::Dict)
	...
	if haskey(inputs, "dfCO2Cap_slack")
		if inputs["NumScenarios"] <= 1
			...
			@variable(EP, vCO2Cap_slack[cap = 1:inputs["NCO2Cap"],s=1:inputs["NumScenarios"]]>=0)
			@expression(EP, eCCO2Cap_slack[cap = 1:inputs["NCO2Cap"],s=1:inputs["NumScenarios"]], 
			inputs["dfCO2Cap_slack"][cap,:PriceCap] * EP[:vCO2Cap_slack][cap,s])
			@expression(EP, eCTotalCO2CapSlack, 
			sum(EP[:eCCO2Cap_slack][cap,s] for cap = 1:inputs["NCO2Cap"], s=1:inputs["NumScenarios"]))
			EP[:eObj] += eCTotalCO2CapSlack
		end
	else 
		if inputs["NumScenarios"] <= 1
	...
	if setup["CO2Cap"] == 1
		if inputs["NumScenarios"] <= 1
			# CO2 cap based on a single scenario
			...
			# Breaking up into the timeseries into NumScenarios scenarios
			# If scenarios are uneven, make the first scenario longest
			scenario_times = timeseries2scenarios(T, inputs["NumScenarios"])
			@constraint(EP, cCO2Emissions_systemwide[cap=1:inputs["NCO2Cap"],s=1:inputs["NumScenarios"]],
			sum((inputs["omega"][t]/inputs["scenario_probability"][s]) * EP[:eEmissionsByZone][z,t] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=scenario_times[s,1]:scenario_times[s,2]) -
			vCO2Cap_slack[cap,s] <=
			sum(inputs["dfMaxCO2"][z,cap] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap])))
		end
