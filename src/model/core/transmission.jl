	...
	if NetworkExpansion == 1
		...
		if MultiStage == 1 
			...
			if haskey(inputs, "dfMaxPathTrans")
				...
				@constraint(EP,cMaxPathFlowPossible[i in 1:length(constrained_paths)], sum(eAvail_Trans_Cap[l] for l in intersect(EXPANSION_LINES,findall(inputs["TransmissionPath"].==constrained_paths[i]))) <= dfMaxPathTrans[i,:Path_Max_Flow_Possible])
			end
		end
