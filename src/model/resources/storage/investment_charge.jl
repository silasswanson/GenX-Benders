...
function investment_charge!(EP::Model, inputs::Dict, setup::Dict)
	...
	if MultiStage == 1
		...
		@expression(EP, eExistingCapCharge[y in STOR_ASYMMETRIC], dfGen[y,:Existing_Charge_Cap_MW]*EP[:vZERO])
	end

	@expression(EP, eTotalCapCharge[y in STOR_ASYMMETRIC],
		if (y in intersect(NEW_CAP_CHARGE, RET_CAP_CHARGE))
			eExistingCapCharge[y] + dfGen[y,:Cap_Size]*(EP[:vCAPCHARGE][y] - EP[:vRETCAPCHARGE][y])
		elseif (y in setdiff(NEW_CAP_CHARGE, RET_CAP_CHARGE))
			eExistingCapCharge[y] + dfGen[y,:Cap_Size]*EP[:vCAPCHARGE][y]
		elseif (y in setdiff(RET_CAP_CHARGE, NEW_CAP_CHARGE))
			eExistingCapCharge[y] -dfGen[y,:Cap_Size]*EP[:vRETCAPCHARGE][y]
		else
	...
	@expression(EP, eCFixCharge[y in STOR_ASYMMETRIC],
		if y in NEW_CAP_CHARGE # Resources eligible for new charge capacity
			dfGen[y,:Inv_Cost_Charge_per_MWyr]*dfGen[y,:Cap_Size]*vCAPCHARGE[y] + dfGen[y,:Fixed_OM_Cost_Charge_per_MWyr]*eTotalCapCharge[y]
		else
	...
	@constraint(EP, cMaxRetCharge[y in RET_CAP_CHARGE], dfGen[y,:Cap_Size]*vRETCAPCHARGE[y] <= eExistingCapCharge[y])
