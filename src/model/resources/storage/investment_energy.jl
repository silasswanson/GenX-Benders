...
function investment_energy!(EP::Model, inputs::Dict, setup::Dict)
	...
	if MultiStage == 1
		...
		@expression(EP, eExistingCapEnergy[y in STOR_ALL], dfGen[y,:Existing_Cap_MWh]*EP[:vZERO])
	end

	@expression(EP, eTotalCapEnergy[y in STOR_ALL],
		if (y in intersect(NEW_CAP_ENERGY, RET_CAP_ENERGY))
			eExistingCapEnergy[y] + dfGen[y,:Cap_Size]*(EP[:vCAPENERGY][y] - EP[:vRETCAPENERGY][y])
		elseif (y in setdiff(NEW_CAP_ENERGY, RET_CAP_ENERGY))
			eExistingCapEnergy[y] + dfGen[y,:Cap_Size]*EP[:vCAPENERGY][y]
		elseif (y in setdiff(RET_CAP_ENERGY, NEW_CAP_ENERGY))
			eExistingCapEnergy[y] - dfGen[y,:Cap_Size]*EP[:vRETCAPENERGY][y]
		else
	...
	@expression(EP, eCFixEnergy[y in STOR_ALL],
		if y in NEW_CAP_ENERGY # Resources eligible for new capacity
			dfGen[y,:Inv_Cost_per_MWhyr]*dfGen[y,:Cap_Size]*vCAPENERGY[y] + dfGen[y,:Fixed_OM_Cost_per_MWhyr]*eTotalCapEnergy[y]
		else
	...
	@constraint(EP, cMaxRetEnergy[y in RET_CAP_ENERGY], dfGen[y,:Cap_Size]*vRETCAPENERGY[y] <= eExistingCapEnergy[y])
