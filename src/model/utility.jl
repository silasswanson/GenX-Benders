#Function below is the same as circular_index(r) = circshift(IndexSet,h)[r] if we had only one scenario.		
function circular_index(r,IndexSet,NScenarios,h)
    n = Int(length(IndexSet)/NScenarios);
    IndexSet_S = [IndexSet[(i-1)*n+1:i*n] for i in 1:NScenarios];
    ir = findfirst(in.(r,IndexSet_S));
    return circshift(IndexSet_S[ir],h)[findfirst(IndexSet_S[ir].==r)]
end
